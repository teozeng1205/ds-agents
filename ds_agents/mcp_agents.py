...
class ProviderAuditMCPAgent(BaseMCPAgent):
    ...
    def __init__(self) -> None:
        instructions = (
            ...
            "3. Prefer the provided tools instead of writing custom SQL:\n"
            "   • `issue_scope_combined_all` returns the full scope (obs_hour, pos, triptype, LOS, O&D, cabin, depart periods, travel DOW, issue_label).\n"
            "   • `issue_scope_combined` (or `_flex`) lets you specify a smaller `dims` list; the flex version can infer provider/site from the request text.\n"
            "   Always include the original question in the `request` argument when using *_flex tools so the server can infer missing context.\n"
